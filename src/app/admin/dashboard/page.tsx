...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Time Range Selector & Location View Toggle */}
      <div className="flex flex-wrap justify-between items-center gap-4">
        <div className="flex gap-2 bg-bg-dark/50 rounded-lg border border-gold/20 p-1">
          <button
            onClick={() => setLocationView("constituency")}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${locationView === "constituency"
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${locationView === "ward"
              ? "bg-gold text-bg-dark"
              : "text-text-dim hover:text-gold"
              }`}
          >
        ...
        {selectedConstituency && (
          <button
            ...
            className="px-4 py-2 rounded-lg border border-gold/20 text-gold hover:bg-gold/10 transition-colors flex items-center gap-2 text-sm"
          >
            <X className="h-4 w-4" />
            Clear Filter: {selectedConstituency}
          </button>
        )}
        <select
          value={timeRange}
      ...
      {/* Stats Grid - Dynamic based on filtered data */}
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center justify-between">
            <Users className="h-5 w-5 text-gold" />
            <span className="text-2xl font-bold text-text-light">{filteredStats.total.toLocaleString()}</span>
          </div>
        ...
        <div className="bg-bg-card/50 rounded-xl border border-green-500/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-green-400">{filteredStats.verified.toLocaleString()}</span>
          </div>
        ...
        <div className="bg-bg-card/50 rounded-xl border border-yellow-500/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-yellow-400">{filteredStats.pending.toLocaleString()}</span>
          </div>
        ...
        <div className="bg-bg-card/50 rounded-xl border border-blue-500/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-blue-400">{filteredStats.phoneVerified.toLocaleString()}</span>
          </div>
        ...
        <div className="bg-bg-card/50 rounded-xl border border-purple-500/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-purple-400">{Math.round(filteredStats.humanVerificationRate)}%</span>
          </div>
        ...
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-text-light">+{stats.thisWeek}</span>
          </div>
          <p className="text-xs text-text-dim mt-1">This Week</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-montserrat text-lg font-bold text-text-light">
              {locationView === "constituency" ? "Constituency" : "Ward"} Distribution
            </h3>
            <div className="flex items-center gap-2 text-xs text-text-dim">
              <span className="inline-flex items-center gap-1">
                <Building2 className="h-3 w-3 text-gold" />
                {locationView === "constituency" ? `${stats.constituencies} constituencies` : `${stats.wards} wards`}
              </span>
            </div>
          </div>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis type="number" tick={{ fill: '#888888', fontSize: 10 }} />
                  <YAxis
                    type="category"
                    dataKey="name"
              ...
              {wardData.length > 15 && (
                <div className="text-center text-[10px] text-text-dim mt-2">
                  Showing top 15 wards of {wardData.length} total. Others grouped as "Others"
                </div>
              )}
            </div>
          )}
      ...
      {/* Charts Row 2 - Risk & Verification */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-6">
          <h3 className="font-montserrat text-lg font-bold text-text-light mb-4">Risk Assessment</h3>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <RePieChart>
                <Pie
        ...
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-6">
          <h3 className="font-montserrat text-lg font-bold text-text-light mb-4">Verification Status</h3>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <RePieChart>
                <Pie
        ...
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-6">
          <h3 className="font-montserrat text-lg font-bold text-text-light mb-4">Device Distribution</h3>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <RePieChart>
                <Pie
      ...
      {/* Charts Row 3 - Bot Score & Event Performance */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={eventData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#2a2a2a" />
                <XAxis dataKey="name" tick={{ fill: '#888888', fontSize: 9 }} interval={0} angle={-15} textAnchor="end" height={60} />
                <YAxis tick={{ fill: '#888888', fontSize: 10 }} />
                <Tooltip content={<CustomTooltip />} />
                <Legend />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center gap-2 text-text-dim">
            ...
            <span className="text-xs">Constituencies</span>
          </div>
          <p className="text-2xl font-bold text-text-light mt-1">{stats.constituencies}</p>
        </div>
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center gap-2 text-text-dim">
          ...
          <p className="text-2xl font-bold text-text-light mt-1">{stats.wards}</p>
        </div>
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center gap-2 text-text-dim">
          ...
          <p className="text-2xl font-bold text-text-light mt-1">{stats.eventsCount}</p>
        </div>
        <div className="bg-bg-card/50 rounded-xl border border-gold/20 p-4">
          <div className="flex items-center gap-2 text-text-dim">
            <Users className="h-4 w-4 text-gold" />
            <span className="text-xs">Event Registrations</span>
          </div>
          <p className="text-2xl font-bold text-text-light mt-1">{stats.eventRegistrations}</p>
        </div>
      </div>
      ...
      <div className="bg-bg-card/50 rounded-xl border border-gold/20 overflow-hidden">
        <div className="p-4 border-b border-gold/20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-text-dim">{displaySupporters.length} supporters</span>
            {selectedConstituency && (
              <button
                onClick={() => setSelectedConstituency(null)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {displaySupporters.slice(0, 15).map((supporter) => {
                ...
                return (
                  <tr key={supporter.id} className="border-t border-gold/10 hover:bg-gold/5 transition-colors">
                    ...
                    <td className="p-3 text-text-dim text-sm">
                      ...
                      {supporter.ward && (
                        <span className="block text-[10px] text-text-dim/50">{supporter.ward}</span>
                      )}
                    </td>
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="p-3">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs ${bg} ${color}`}>
                        {risk === 'High Risk' ? <AlertTriangle className="h-3 w-3" /> :
                          risk === 'Medium Risk' ? <Clock className="h-3 w-3" /> :
        ...
        {displaySupporters.length > 15 && (
          <div className="p-4 border-t border-gold/20 text-center">
            <button className="text-gold text-sm hover:underline">View All {displaySupporters.length} Supporters</button>
          </div>
        )}
      </div>
    </div>
